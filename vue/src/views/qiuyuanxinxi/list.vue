<template>
    <div class="views-qiuyuanxinxi-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 球员信息查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="shengao" label="身高/cm" >
                        <template #default="{ row }"> {{ row.shengao }} </template>
                    </el-table-column>
                    <el-table-column prop="tizhong" label="体重/kg" >
                        <template #default="{ row }"> {{ row.tizhong }} </template>
                    </el-table-column>
                    <el-table-column prop="suoshuqiudui" label="所属球队">
                        <template #default="{ row }"> {{ row.suoshuqiudui }} </template>
                    </el-table-column>
                    <el-table-column prop="qiuyuanweizhi" label="球员位置" >
                        <template #default="{ row }"> {{ row.qiuyuanweizhi }} </template>
                    </el-table-column>
                    <el-table-column prop="qiuyihao" label="球衣号" >
                        <template #default="{ row }"> {{ row.qiuyihao }} </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{ row }">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/qiuyuanxinxi/detail?id=' + row.id)">详情</el-button> </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/qiuyuanxinxi/updt?id=' + row.id)">编辑</el-button> </el-tooltip>
...
<script setup>
    ...
    import { canQiuyuanxinxiSelect, useQiuyuanxinxiSelect, canQiuyuanxinxiDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canQiuyuanxinxiDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/qiuyuanxinxi/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
