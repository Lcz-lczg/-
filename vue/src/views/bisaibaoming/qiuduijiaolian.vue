<template>
    <div class="views-bisaibaoming-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 比赛报名查询 </span>
                    </div>
                </template>
                ...
                <div class="form-search">
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        ...
                        <el-form-item label="球队队员">
                            <el-input v-model="search.qiuduiduiyuan"></el-input>
                        </el-form-item>
                        <el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="qiuduiduiyuan" label="球队队员">
                        <template #default="{ row }"> {{ row.qiuduiduiyuan }} </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="lianxifangshi" label="联系方式" >
                        <template #default="{ row }"> {{ row.lianxifangshi }} </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{ row }">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/bisaibaoming/detail?id=' + row.id)">详情</el-button> </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/bisaibaoming/updt?id=' + row.id)">编辑</el-button> </el-tooltip>
...
<script setup>
    ...
    import { canBisaibaomingSelect, useBisaibaomingSelect, canBisaibaomingDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canBisaibaomingDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/bisaibaoming/selectQiuduijiaolian", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
