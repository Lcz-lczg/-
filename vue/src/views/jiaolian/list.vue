<template>
    <div class="views-jiaolian-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 教练查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="gonghao" label="工号" >
                        <template #default="{ row }"> {{ row.gonghao }} </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="zizhizhengshu" label="资质证书" >
                        <template #default="{ row }"> <e-img :src="row.zizhizhengshu" style="max-width: 120px" /> </template>
                    </el-table-column>
                    <el-table-column prop="zhijiaojingyan" label="执教经验">
                        <template #default="{ row }"> {{ row.zhijiaojingyan }} </template>
                    </el-table-column>
                    <el-table-column prop="lianxidianhua" label="联系电话" >
                    ...
                    <el-table-column  label="状态">
                        <template #default="{ row }">
                            ...
                            <el-button size="small" :type="row.issh == '是' ? 'success' : 'danger'" @click="canJiaolianCheckIssh(row)">
                                {{ row.issh == "是" ? "解封" : "封禁" }}
                            </el-button>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{ row }">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/jiaolian/detail?id=' + row.id)">详情</el-button> </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/jiaolian/updt?id=' + row.id)">编辑</el-button> </el-tooltip>
...
<script setup>
    ...
    import { canJiaolianSelect, useJiaolianSelect, canJiaolianDelete, canJiaolianCheckIssh } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canJiaolianDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/jiaolian/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
