                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{ row }">
                            <el-button-group>
                                <el-button size="small" @click="$router.push('/' + row.biao + '/detail?id=' + row.biaoid)">详情</el-button><el-tooltip effect="dark" content="编辑" placement="top-start"><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/pinglun/updt?id=' + row.id)">编辑</el-button> </el-tooltip>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/pinglun/selectPinglunren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
