<template>
    <div class="views-dianzan-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 点赞查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row @selection-change="handleSelectionChange">
                    ...
                    <el-table-column prop="addtime" label="点赞时间">
                    ...
                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{ row }">
                            <el-button-group>
                                <el-button size="small" @click="$router.push('/' + row.biao + '/detail?id=' + row.biaoid)">详情</el-button><el-tooltip effect="dark" content="删除" placement="top-start"><el-button type="danger" :icon="Delete" size="small" @click="deleteItems(row.id)">删除</el-button> </el-tooltip>
                            </el-button-group>
                        </template>
                    </el-table-column>
                </el-table>
...
<script setup>
    ...
    import { canDianzanSelect, useDianzanSelect, canDianzanDelete } from "@/module";
    ...
    const loadList = (page) => {
        ...
        http.post("/api/dianzan/selectUsername", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
