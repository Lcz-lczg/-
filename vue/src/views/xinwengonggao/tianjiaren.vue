<template>
    <div class="views-xinwengonggao-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 新闻公告查询 </span>
                    </div>
                </template>
                ...
                <div class="form-search">
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        ...
                        <el-form-item label="分类">
                            <el-select v-model="search.fenlei"><el-option label="请选择" value=""></el-option><e-select-option type="option" module="xinwenfenlei" value="id" label="fenleimingcheng"></e-select-option></el-select>
                        </el-form-item>
                        <el-form-item label="添加人">
                            <el-input v-model="search.tianjiaren"></el-input>
                        </el-form-item>
                        <el-form-item label="点击量">
                            <el-input type="text" style="width: 80px" v-model="search.dianjiliang_start"></el-input>
                            -
                            <el-input type="text" style="width: 80px" v-model="search.dianjiliang_end"></el-input>
                        </el-form-item>
                        <el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="fenlei" label="分类" >
                        <template #default="{ row }"> <e-select-view module="xinwenfenlei" :value="row.fenlei" select="id" show="fenleimingcheng"></e-select-view> </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="dianjiliang" label="点击量" >
                        <template #default="{ row }"> {{ row.dianjiliang }} </template>
                    </el-table-column>

                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{ row }">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/xinwengonggao/detail?id=' + row.id)">详情</el-button> </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/xinwengonggao/updt?id=' + row.id)">编辑</el-button> </el-tooltip>
...
<script setup>
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canXinwengonggaoDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/xinwengonggao/selectTianjiaren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
