<template>
    <div class="views-guanzhong-list">
        <div>
            <el-card class="box-card">
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="yonghuming" label="用户名" >
                        <template #default="{ row }"> {{ row.yonghuming }} </template>
                    </el-table-column>
                    <el-table-column prop="mima" label="密码">
                        <template #default="{ row }"> {{ row.mima }} </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{ row }">
                            <el-button-group>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/guanzhong/updt?id=' + row.id)">编辑</el-button> </el-tooltip>
...
<script setup>
    ...
    import { canGuanzhongSelect, useGuanzhongSelect, canGuanzhongDelete, canGuanzhongCheckIssh } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canGuanzhongDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/guanzhong/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
