<template>
    <div class="views-bisaixinxi-index">
        <div>
            <e-container>
                    ...
                    <div class="sidebar-widget-body">
                        <form action="javascript:;" @submit="searchSubmit" class="form-search">
                            <table class="jd-search">
                                <tbody>
                                    <tr>
                                        <td class="label">比赛标题</td>
                                        <td>
                                            <el-input type="text" style="width: 150px" v-model="search.bisaibiaoti" placeholder="请输入关键词"> </el-input>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="label">比赛类型</td>
                                        <td>
                                            <p class="search-radio">
                                                <a href="javascript:;" @click="selectRadio('bisaileixing', '')" :class="{ active: !search.bisaileixing }">全部</a>
                                                <a href="javascript:;" v-for="r in mapbisaileixing2" @click="selectRadio('bisaileixing', r.id)" :class="{ active: search.bisaileixing == r.id }" v-text="r.leixingmingcheng"> </a>
                                            </p>
                                        </td>
                                    </tr>
                                    <tr>
                        ...
                        <el-row :gutter="30">
                            <el-col v-for="r in lists" :md="6" :key="r.id" style="margin-bottom: 20px">
                                <article class="single_product v-module-xiezi">
                                    <figure>
                                        <div class="product_thumb" @click="$goto('/bisaixinxi/detail?id=' + r.id)">
                                            <div class="label_product"></div>
                                            <e-img class="primary_img" :src="r.bisaitupian" pb="100" :is-scale="true" />
                                        </div>
                                        <figcaption class="product_content">
                                            <h4 class="product_name">
                                                <a href="javascript:;" @click="$goto('/bisaixinxi/detail?id=' + r.id)"> {{ r.bisaibiaoti }}</a>
                                            </h4>
                                            <div class="add_to_cart">
                                                <a href="javascript:;" title="查看详情" @click="$goto('/bisaixinxi/detail?id=' + r.id)">查看详情</a>
                                            </div>
                                        </figcaption>
                                    </figure>
                                </article>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/bisaixinxi/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
    ...
    const mapbisaileixing2 = DB.name("bisaileixing").field("id,leixingmingcheng").order("id desc").selectRef();
</script>

<style scoped lang="scss">
    .views-bisaixinxi-index {
    }
</style>
