...
public class UserService {
    ...
    public R<Object> login(String username, String pwd, String cx) {
        ...
        // 判断是否为"观众"角色登录时则使用这个进行登录
        if (cx.equals("观众")) {
            // 通过GuanzhongService.login 方法进行登录

            Guanzhong user = guanzhongService.login(username, pwd);
            // user 为null 则提示用户账号或密码错误
            if (user == null) {
                return R.error("账号或密码错误");
            }
            ...
            // 设置table 变量
            // 将bean 实体类转化为session
            session = guanzhongService.toSession(user);
        }
        // 判断是否为"教练"角色登录时则使用这个进行登录
        if (cx.equals("教练")) {
            // 通过JiaolianService.login 方法进行登录

            Jiaolian user = jiaolianService.login(username, pwd);
            // user 为null 则提示用户账号或密码错误
            if (user == null) {
                return R.error("账号或密码错误");
            }
            ...
            // 设置table 变量
            // 将bean 实体类转化为session
            session = jiaolianService.toSession(user);
        }
        // 判断是否为"球员信息"角色登录时则使用这个进行登录
        if (cx.equals("球员信息")) {
            // 通过QiuyuanxinxiService.login 方法进行登录

            Qiuyuanxinxi user = qiuyuanxinxiService.login(username, pwd);
            // user 为null 则提示用户账号或密码错误
            if (user == null) {
                return R.error("账号或密码错误");
            }
            ...
            // 设置table 变量
            // 将bean 实体类转化为session
            session = qiuyuanxinxiService.toSession(user);
        }
    ...
    public R<Object> editPassword(PasswordPojo passwordPojo) {
        ...
        if (roles.equals("观众")) {
            return guanzhongService.editPassword(SessionFactory.getId(), passwordPojo.getOldPassword(), passwordPojo.getNewPassword());
        }

        if (roles.equals("教练")) {
            return jiaolianService.editPassword(SessionFactory.getId(), passwordPojo.getOldPassword(), passwordPojo.getNewPassword());
        }

        if (roles.equals("球员信息")) {
            return qiuyuanxinxiService.editPassword(SessionFactory.getId(), passwordPojo.getOldPassword(), passwordPojo.getNewPassword());
        }
    ...
    public R select(@RequestBody Map<String, Object> data) {
        ...
        // 仅允许 SELECT 查询，禁止执行 UPDATE/DELETE/INSERT 等
        if (!"select".equals(type)) {
            return R.error("非法的操作类型");
        }

        String sql = String.valueOf(data.get("sql"));
        // 二次校验：只允许 SELECT 开头的语句（严格正则，防止 SELECT/*comment*/ 绕过）
        if (!sql.trim().matches("(?si)^\\s*select\\s+.*")) {
            return R.error("仅允许 SELECT 查询");
        }
