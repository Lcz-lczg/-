...
@Service
public class BisaibaomingService {

    // 获取数据库操作类mapper
    @Resource
    private BisaibaomingMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Bisaibaoming> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Bisaibaoming>> selectAll(Wrapper<Bisaibaoming> query) {
    ...
    public R<List<Bisaibaoming>> selectAll() {
        QueryWrapper<Bisaibaoming> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Bisaibaoming> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Bisaibaoming> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Bisaibaoming> wrapper = Wrappers.query();
        ...
        if (!StringUtil.isNullOrEmpty(map.get("baomingbianhao"))) {
            wrapper.like("baomingbianhao", map.get("baomingbianhao"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("baomingqiudui"))) {
            wrapper.like("baomingqiudui", map.get("baomingqiudui"));
        }
    ...
    public R selectPages(QueryWrapper<Bisaibaoming> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Bisaibaoming entityData, Map post) {
        // 判断是否有填写报名编号。
        if (StringUtil.isNullOrEmpty(post.get("baomingbianhao"))) {
            return R.error("请填写报名编号");
        }
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Bisaibaoming entityData, Map post) {
        // 判断是否有填写报名编号。
        if (StringUtil.isNullOrEmpty(post.get("baomingbianhao"))) {
            return R.error("请填写报名编号");
        }
