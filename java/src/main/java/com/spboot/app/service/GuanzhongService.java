...
import com.spboot.app.mapper.GuanzhongMapper;
...
@Service
public class GuanzhongService {

    // 获取数据库操作类mapper
    @Resource
    private GuanzhongMapper mapper;
     ...
     * 将数据转换成Session对象
     */
    public Session toSession(Guanzhong user) {
        // 创建Session 对象
        Session session = new Session();
        // 设置相应数据进session 对象
        session.setId(user.getId());
        session.setUsername(user.getYonghuming());
        session.setTable("guanzhong");
        session.setRoles("观众");
     ...
     * @return 观众对象实体类
     */
    public Guanzhong login(String username, String pwd) {
        Guanzhong row = findByYonghuming(username);
    ...
    public R<Object> editPassword(int id, String oldPassword, String newPassword) {
        ...
        Guanzhong admins = mapper.selectById(id);
    ...
    /**
     *   根据Yonghuming字段参数获取一行数据
     */
    public Guanzhong findByYonghuming(String username) {
        // 新建观众模块实体类Guanzhong
        Guanzhong pojo = new Guanzhong();
        // 设置参数
        pojo.setYonghuming(username);
        // 根据实体类新建QueryWrapper查询条件类
        QueryWrapper<Guanzhong> queryWrapper = Wrappers.query(pojo);
        Guanzhong row = mapper.selectOne(queryWrapper);
        return row;
    }

    /**
     *   根据Yonghuming字段参数获取一行数据，并不包含某uid 参数得行
     */
    public Guanzhong findByYonghuming(String username, Integer uid) {
        // 新建观众模块实体类Guanzhong
        Guanzhong pojo = new Guanzhong();
        // 设置参数
        pojo.setYonghuming(username);
     ...
     *  根据id 获取一行数据
     */
    public R<Guanzhong> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Guanzhong>> selectAll(Wrapper<Guanzhong> query) {
     ...
     *  直接筛选所有数据
     */
    public R<List<Guanzhong>> selectAll() {
        QueryWrapper<Guanzhong> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Guanzhong> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Guanzhong> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Guanzhong> wrapper = Wrappers.query();
        ...
        if (!StringUtil.isNullOrEmpty(map.get("yonghuming"))) {
            wrapper.like("yonghuming", map.get("yonghuming"));
        }
    ...
    public R selectPages(QueryWrapper<Guanzhong> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Guanzhong entityData, Map post) {
        // 判断是否有填写用户名。
        if (StringUtil.isNullOrEmpty(post.get("yonghuming"))) {
            return R.error("请填写用户名");
        }
        // 判断是否有填写用户名,有则判断是否在数据中已经存在，存在则报错。
        if (findByYonghuming(entityData.getYonghuming()) != null) {
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Guanzhong entityData, Map post) {
        // 判断是否有填写用户名。
        if (StringUtil.isNullOrEmpty(post.get("yonghuming"))) {
            return R.error("请填写用户名");
        }
        // 判断是否有填写用户名,有则判断是否在数据中已经存在，存在则报错。
        if (findByYonghuming(entityData.getYonghuming(), entityData.getId()) != null) {
            return R.error("用户名已经存在");
        }
        // 判断是否有填写姓名。
        if (StringUtil.isNullOrEmpty(post.get("xingming"))) {
        ...
        if (!StringUtil.isNullOrEmpty(currentPwd)) {
            ...
            Guanzhong old = mapper.selectById(entityData.getId());
    ...
    public R<Object> checkIssh(Integer id, String value) {
        Guanzhong row = mapper.selectById(id);
        row.setIssh(value);
