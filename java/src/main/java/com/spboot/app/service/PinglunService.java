...
public class PinglunService {
     ...
     *  根据id 获取一行数据
     */
    public R<Pinglun> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Pinglun>> selectAll(Wrapper<Pinglun> query) {
    ...
    public R<List<Pinglun>> selectAll() {
        QueryWrapper<Pinglun> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Pinglun> wrapper = mapToWrapper(map);
        // 设置排序
    ...
    /**
     *  根据map 条件筛选pinglunren字段等于session.username的数据并分页
     *
     */
    public R selectPagesPinglunren(Map<String, Object> map) {
        // 获取前端给到列表基础参数
        SelectPage selectPage = new SelectPage(map, 10, "id", "DESC");
        // 将map参数转换为mybatis-plus的QueryWrapper类
        QueryWrapper<Pinglun> wrapper = mapToWrapper(map);
        // 设置为当前角色
        wrapper.eq("pinglunren", SessionFactory.getUsername());
        wrapper.orderBy(true, selectPage.isAsc(), selectPage.getOrderby());
        Page page = new Page(selectPage.getPage(), selectPage.getPagesize());
        return selectPages(wrapper, page);
    }
     ...
     */
    public QueryWrapper<Pinglun> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Pinglun> wrapper = Wrappers.query();
    ...
    public R selectPages(QueryWrapper<Pinglun> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Pinglun entityData, Map post) {
        // 判断是否有填写评分。
        if (StringUtil.isNullOrEmpty(post.get("pingfen"))) {
        ...
        // 确保保存评论人姓名和头像
        if (post.get("xingming") != null) {
            entityData.setXingming(post.get("xingming").toString());
        }
        if (post.get("touxiang") != null) {
            entityData.setTouxiang(post.get("touxiang").toString());
        }
        if (post.get("pinglunren") != null) {
            entityData.setPinglunren(post.get("pinglunren").toString());
        }
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Pinglun entityData, Map post) {
        // 判断是否有填写评分。
        if (StringUtil.isNullOrEmpty(post.get("pingfen"))) {
            return R.error("请填写评分");
        }
