...
@Service
public class BisaixinxiService {

    // 获取数据库操作类mapper
    @Resource
    private BisaixinxiMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Bisaixinxi> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Bisaixinxi>> selectAll(Wrapper<Bisaixinxi> query) {
    ...
    public R<List<Bisaixinxi>> selectAll() {
        QueryWrapper<Bisaixinxi> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Bisaixinxi> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Bisaixinxi> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Bisaixinxi> wrapper = Wrappers.query();
        ...
        if (!StringUtil.isNullOrEmpty(map.get("bisaileixing"))) {
            wrapper.eq("bisaileixing", map.get("bisaileixing"));
        }
    ...
    public R selectPages(QueryWrapper<Bisaixinxi> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Bisaixinxi entityData, Map post) {
        // 判断是否有填写比赛编号。
        if (StringUtil.isNullOrEmpty(post.get("bisaibianhao"))) {
            return R.error("请填写比赛编号");
        }
        // 判断是否有填写比赛标题。
        if (StringUtil.isNullOrEmpty(post.get("bisaibiaoti"))) {
            return R.error("请填写比赛标题");
        }
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Bisaixinxi entityData, Map post) {
        // 判断是否有填写比赛编号。
        if (StringUtil.isNullOrEmpty(post.get("bisaibianhao"))) {
            return R.error("请填写比赛编号");
        }
